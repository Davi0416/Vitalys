...
export default function Inicio() {
  const { pacientes, profissionais, agendamentos,
          carregarPacientes, carregarProfissionais, carregarAgendamentos,
          nomePaciente, nomeProfissional } = useData();
  ...
  const proximos = agHoje.slice().sort((a, b) => new Date(a.dataEHoraMarcadas) - new Date(b.dataEHoraMarcadas));
    ...
    <div>
      ...
      <div className="inicio-cards">
        <Card
          ...
          valor={agHoje.length}
          sub={agHoje.length > 0 ? `${confirmados} confirmados · ${pendentes} pendentes · ${cancelados} cancelados` : 'Nenhum agendamento hoje'}
      ...
      <div className="inicio-paineis">
        ...
        <div className="inicio-painel painel-atend">
          <div className="painel-header">
            ...
            <a href="#" className="painel-link" onClick={e => { e.preventDefault(); navigate('/agendamentos'); }}>
              Ver agenda completa
            </a>
          </div>
          {proximos.length === 0 ? (
      ...
      {modalAgend && (
        <ModalAgendamento
          ...
          onSalvo={() => { setModalAgend(false); carregarAgendamentos(); }}
        />
      )}
    </div>
  );
...
function Card({ label, valor, sub, icon }) {
  return (
