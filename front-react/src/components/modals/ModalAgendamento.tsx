...
export default function ModalAgendamento({ onClose, onSalvo }: Props) {
  ...
  const [autocomplete, setAuto] = useState<Paciente[]>([]);
  ...
  useEffect(() => {
    if (!pacientes.length)     void carregarPacientes();
    if (!profissionais.length) void carregarProfissionais();
  }, []);

  function filtrarPac(v: string) {
    setBuscaPac(v);
    setPacSel(null);
  ...
  function selecionarPac(p: Paciente) {
    setBuscaPac(p.nome);
    setPacSel(p);
    setAuto([]);
  }

  const ocupados: string[] = (profId && data)
    ? agendamentos
        .filter(a =>
          String(a.idProfissional) === String(profId) &&
