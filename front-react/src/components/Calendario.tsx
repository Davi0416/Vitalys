...
export default function Calendario() {
  ...
  useEffect(() => {
    if (!agendamentos.length)  void carregarAgendamentos();
  ...
  const diasComAtend = new Set<number>(
    agendamentos
      .filter(a => a.dataEHoraMarcadas)
      .map(a => {
        const d = new Date(a.dataEHoraMarcadas);
        return d.getMonth() === mes && d.getFullYear() === ano ? d.getDate() : null;
      })
      .filter((d): d is number => d !== null)
  );

  const agendDia = diaSel !== null
